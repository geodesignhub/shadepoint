...
def upgrade():
    ...
    op.create_index(op.f('ix_measuretype_name'), 'measuretype', ['name'], unique=True)
    ...
    sa.Column('geometry', Geometry(geometry_type='GEOMETRY', srid=4326, spatial_index=False, from_text='ST_GeomFromEWKT', name='geometry'), nullable=True, spatial_index=False),
    sa.Column('measure_id', sa.String(), nullable=True),
    sa.Column('styling', JSON(), nullable=True),
    sa.Column('physical_properties', JSON(), nullable=True),
    sa.ForeignKeyConstraint(['measure_id'], ['measuretype.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    op.create_geospatial_index('idx_nbs_geometry', 'naturebasedsolution', ['geometry'], unique=False, postgresql_using='gist', postgresql_ops={})
    ...
    sa.Column('map_settings', JSON(), nullable=True),
    sa.Column('targets', JSON(), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_project_title'), 'project', ['title'], unique=False)

    op.create_table('project_nbs',
...
def downgrade():
    ...
    op.drop_geospatial_index('idx_nbs_geometry', table_name='naturebasedsolution', postgresql_using='gist', column_name='geometry')
    ...
    op.drop_index(op.f('ix_measuretype_name'), table_name='measuretype')
    op.drop_table('measuretype')
