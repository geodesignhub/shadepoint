...
def existing_buildings_notify_shadow_complete(job, connection, result, *args, **kwargs):
    ...
    app = create_app()      
    with app.app_context():
        sse.publish({"shadow_key": job_id}, type='existing_buildings_shadow_generation_success')
        
def existing_buildings_shadow_generation_failure(job, connection, type, value, traceback):
    print('Job with %s failed..'% str(job.id))
...
def notify_gdh_roads_shadow_intersection_complete(job, connection, result, *args, **kwargs):
    ...
    print('Job with id %s completed the shadow intersection successfully..'% str(job.id))

        
def notify_gdh_roads_shadow_intersection_failure(job, connection, type, value, traceback):
    print('Job with %s failed..'% str(job.id))
...
def notify_existing_roads_shadow_intersection_complete(job, connection, result, *args, **kwargs):
    ...
    app = create_app()      
    with app.app_context():
        sse.publish({"existing_buildings_roads_shadow_stats_key": job_id}, type='existing_buildings_roads_shadow_complete')

    print('Job with id %s completed the existing building shadow intersection successfully..'% str(job.id))

        
def notify_existing_roads_shadow_intersection_failure(job, connection, type, value, traceback):
    print('Job with %s failed..'% str(job.id))
...
def notify_buildings_download_complete(job, connection, result, *args, **kwargs):
    ...
    app = create_app()      
    with app.app_context():
        sse.publish({"existing_buildings_key": job_id}, type='existing_buildings_download_success')
